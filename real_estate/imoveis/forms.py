...
class ComprarBuscaForm(forms.Form):
	CIDADES = (('Macaé', 'Macaé'), ('Rio das Ostras', 'Rio das Ostras'), ('Campos', 'Campos'), ('Búzios', 'Búzios'))
	cidade = forms.ChoiceField(choices = CIDADES, label="", initial='Selecione a Cidade',
	 							widget=forms.Select(attrs={'class':'in-drop'}), 
	 							required=True)
	TIPOS_DE_IMOVEL = (('Casa', 'Casa'), ('Apartamento', 'Apartamento'), ('Comercial','Comercial'), ('Fazendo','Fazenda'))
	...
	quartos = forms.ChoiceField(choices = QUARTOS, label="", initial='Selecione o n° de quartos',
	 							widget=forms.Select(attrs={'class':'in-drop'}), 
	 							required=True)
	PRECOS_MINIMO = ((0,'R$ 00,00'), (50000, 'R$ 50.000,00'),(100000, 'R$ 100.000,00'), (200000, 'R$ 200.000,00'), (300000, 'R$ 300.000,00'), (400000, 'R$ 400.000,00'))
	preco_minimo = forms.ChoiceField(choices = PRECOS_MINIMO, label="", initial='Selecione o preço mínimo',
	 							widget=forms.Select(attrs={'class':'in-drop'}), 
	 							required=True)
	PRECOS_MAXIMO = ((1000000, 'R$ 1.000.000,00'), (50000, 'R$ 50.000,00'),(100000, 'R$ 100.000,00'), (200000, 'R$ 200.000,00'), (300000, 'R$ 300.000,00'), (400000, 'R$ 400.000,00'))
...
class AlugarBuscaForm(forms.Form):
	...
	quartos = forms.ChoiceField(choices = QUARTOS, label="", initial='Selecione o n° de quartos',
	 							widget=forms.Select(attrs={'class':'in-drop'}), 
	 							required=True)
	PRECOS_MINIMO = ((0,'R$ 00,00'), (500, 'R$ 500,00'),(1000, 'R$ 1000,00'), (2000, 'R$ 2000,00'), (3000, 'R$ 3000,00'), (4000, 'R$ 4000,00'))
	preco_minimo = forms.ChoiceField(choices = PRECOS_MINIMO, label="", initial='Selecione o preço mínimo',
	 							widget=forms.Select(attrs={'class':'in-drop'}), 
	 							required=True)
	PRECOS_MAXIMO = ((1000, 'R$ 1000,00'), (2000, 'R$ 2000,00'),(3000, 'R$ 3000,00'), (4000, 'R$ 4000,00'), (6000, 'R$ 300.000,00'), (10000, 'R$ 10000,00'))
	preco_maximo = forms.ChoiceField(choices = PRECOS_MAXIMO, label="", initial='Selecione o preço máximo',
	 							widget=forms.Select(attrs={'class':'in-drop'}), 
	 							required=True)
	AREA_MINIMA = ((0, '0m2'), (50, '50m2'),(70, '70m2'), (100, '100m2'))
...
class AnuncioForm(ModelForm):
	class Meta:
		model = Anuncio
		exclude = ['user','slug','pagamento','publicado']
		labels = {
            'titulo': ('Título'),
	...
	def __init__(self, *args, **kwargs):
		...
		for key in self.fields:
			print(self.fields[key].help_text)
			if self.fields[key].help_text:
				self.fields[key].widget.attrs.update({'class': 'has-popover','data-content':self.fields[key].help_text})
